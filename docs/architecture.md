...
Go and Python packages don't consume npm dependencies — `package.json` in those packages is a thin wrapper whose `scripts.{build,dev,lint,test,install}` shell out to native tooling, so `turbo build/dev/lint/test` can drive a polyglot repo uniformly from one root command.

`libs/bash/build-tools` exposes bin scripts (`py-install`, `py-build`, `py-dev`, `py-lint`, implemented in `py-scripts/`). Every Python app/lib declares `lib.bash.build-tools` as a `workspace:*` devDependency and calls those scripts from its own `package.json` scripts. `py-build`/`py-install` create a `.venv`, then use Poetry if `pyproject.toml` exists, else `pip install -r requirements.txt`.
