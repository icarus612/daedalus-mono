...
url = (
    f"https://www.newegg.com/{sys.argv[1]}"
    if len(sys.argv) > 1
    else "https://www.newegg.com/Video-Cards-Video-Devices/Category/ID-38?Tpk=graphics%20card"
)
containers = soup(requests.get(url).content, "html.parser").findAll(
...
for i, x in enumerate(containers):
    ...
    rating = (
        x.find("a", {"class": "item-rating"})["title"]
        if x.find("a", {"class": "item-rating"})
        else ""
    )
