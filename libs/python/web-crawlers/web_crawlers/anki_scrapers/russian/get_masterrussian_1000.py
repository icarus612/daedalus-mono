...
def get_audio_src(u):
    ...
    return soup(requests.get(url).content, "html.parser").find(
...
def build_cards(grp):
    with open("./master-russian.txt", "w") as f_grp:
        f_grp.writelines(
            f'{card["russian"]} | {card["pos"]} | {card["english"]}'
            f' | [sound:{card["file_name"]}] \n'
            for card in grp
        )
...
def build_cw_deck():
    ...
    file_urls = dict(
        [
            (i["file_name"], i["audio_src"])
            for i in cards_lst
            if i["audio_src"] is not None
        ]
    )
