...
def download_lib(lib=lib, version=version):
    page = soup(
        requests.get(f"https://pkg.go.dev/{lib}@go{version}").content, "html.parser"
    )
    decks = {}
    ...
    for el in [i.find("span") for i in page.find_all("h4") if "func" in i.text]:
        ...
        deck_name = (
            el.text.split("(")[1].split(")")[0].replace("*", "")
            if is_method
            else "general"
        )
        ...
        r_string = re.search(rf"{name}(?:\[.*?\])?\(", with_options).group(0)

        params = ", ".join(
            [
                f"<i>{x.split(' ')[0]}</i>"
                for x in with_options.partition(r_string)[2].split(")")[0].split(", ")
            ]
        )
...
if __name__ == "__main__":
    lib_name = argv[1] if len(argv) > 1 else lib
    v = argv[2] if len(argv) > 2 else version

    download_lib(lib=lib_name, version=v)
