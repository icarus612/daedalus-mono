from helpers.jpod101_crawlers import download_audio, search
...
with (
    open("./imported-decks/particles-new.txt") as p_new,
    open("./particles-final.txt", "w") as p_final,
):
    final = []
    update_audio = dict()
    new = split(p_new, "|")
    ...
    retry_missed = search(
        [new[all_words.index(word)][0] for word in search_results[1]]
    )[0]
