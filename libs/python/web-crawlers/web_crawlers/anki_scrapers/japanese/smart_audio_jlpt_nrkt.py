...
current_lst = [
    el
    for el in os.listdir("/home/icarus-64/.local/share/Anki2/User 1/collection.media")
    if "ic_nrkt_" in el
]
all_lst = []
...
fnl_lst = list(
    set(tuple(el) for el in all_lst if el[3].split(":")[1][:-1] not in current_lst)
)
fnl_lst.reverse()
...
for card in fnl_lst:
    ...
    driver.get("https://www.narakeet.com/languages/japanese-text-to-speech/")
    for _ in range(8):
        sleep(1)
        try:
    ...
    for _ in range(8):
        sleep(5)
        status = (
            driver.find_element(By.CSS_SELECTOR, "[data-show-stage]")
            .get_attribute("stage")
            .lower()
            .strip()
        )
        ...
        if status == "finished":
            audio = requests.get(
                driver.find_element(
                    By.CSS_SELECTOR, '[data-prop-link="result"]'
                ).get_attribute("href")
            )
