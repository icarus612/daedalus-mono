...
def get_proxy_list():
    ...
    proxies = [
        [td.get_attribute("innerHTML") for td in tr.find_elements(By.TAG_NAME, "td")][
            :2
        ]
        for tr in driver.find_element(By.TAG_NAME, "table").find_elements(
            By.TAG_NAME, "tr"
        )
    ][1:]
    driver.quit()
...
for proxy in get_proxy_list():
    try:
        ...
        driver.get("https://ttsmp3.com/text-to-speech/Japanese/")
...
for deck_name, deck in raw_json.items():
    ...
    with open(f"./jlptsensei-particles-{deck_name}.txt", "w") as deck_file:
        for card in deck:
            clean_card = card[0].replace("...", "-").replace(" ", "").replace("/", "-")
            file_name = f"ic_ttmp3_{clean_card}.mp3"
            text_area = driver.find_element(By.ID, "voicetext")
            text_area.clear()
            text_area.send_keys(
                card[1].split("/")[0].replace("...", '<break time="700ms"/>').strip()
                + '<break time="100ms"/>'
            )
            ...
            events = [
                event
                for event in [
                    json.loads(entry["message"])["message"] for entry in browser_log
                ]
