...
def search(word_list, key_prefix=False, key_suffix=False):
    ...
    def get_word_html(el, val):
        return el.find_element(By.CLASS_NAME, f"dc-vocab_{val}").get_attribute(
            "innerHTML"
        )
    ...
    for raw_word in word_list:
        ...
        for element in results:
            if word in [
                get_word_html(element, "romanization"),
                get_word_html(element, "kana"),
            ]:
                audio_dict[file_path] = (
                    element.find_element(By.TAG_NAME, "audio")
...
def common_words(end_point="100", email=False, password=False):
    ...
    try:
        ...
        driver.find_element(By.CLASS_NAME, "js-sign-in--a__email-input").send_keys(
            email
        )
        driver.find_element(By.CLASS_NAME, "js-sign-in--a__password-input").send_keys(
            password
        )
    ...
    while True:
        ...
        for element in driver.find_elements(By.CLASS_NAME, "wlv-item"):
            item = dict()
            item["english"] = element.find_element(
                By.CLASS_NAME, "wlv-item__english"
            ).get_attribute("innerHTML")
            try:
                item["higarana"] = (
                    element.find_element(By.CLASS_NAME, "js-wlv-word-field-kana")
                    .find_element(By.CLASS_NAME, "wlv-item__word")
                    .get_attribute("innerHTML")
                )
                item["romaji"] = element.find_element(
                    By.CLASS_NAME, "js-wlv-word-field-romaji"
                ).get_attribute("innerHTML")
                item["audio_src"] = element.find_element(
                    By.TAG_NAME, "audio"
                ).get_attribute("src")
        ...
        try:
            driver.find_element(By.CLASS_NAME, "r101-pagination--b").find_element(
