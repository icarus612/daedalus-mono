...
cards = []
req = soup(
    requests.get("https://numpy.org/doc/stable/reference/arrays.ndarray.html").content,
    "html.parser",
)
...
for url in urls:
    try:
        ...
        full = soup(
            requests.get(f"https://numpy.org/doc/stable/reference/{url}").content,
            "html.parser",
        )
        ...
        print(e)
with open("numpy_arrays.txt", "w") as file:
