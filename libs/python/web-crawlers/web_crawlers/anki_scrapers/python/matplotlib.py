...
cards = []
req = soup(
    requests.get("https://matplotlib.org/stable/api/pyplot_summary.html").content,
    "lxml",
)
urls = [
    i.find("a")["href"]
    for i in req.select("li.toctree-l1.current")[0].find_all(class_="toctree-l2")
]
for url in urls:
    try:
        ...
        full = soup(
            requests.get(f"https://matplotlib.org/stable/api/{url}").content, "html5lib"
        )
        ...
        if len(full_m.find_all("span", {"class": "sig-paren"})) > 0:
            ...
            req_args = list(
                filter(lambda x: all([i not in ["=", "*"] for i in x]), all_args)
            )
        ...
        print(e)
with open("matplotlib.txt", "w") as file:
