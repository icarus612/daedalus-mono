...
def build_and_save(
    word, language, file_name=None, dir_name="audio_files", replace=True
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="RU",
        type=str,
        help="The language to use for the TTS",
    )
    parser.add_argument(
        "--output_dir",
        "-o",
    ...
    parser.add_argument(
        "--split_char", "-s", default="|", type=str, help="The name of the output file"
    )
    ...
    if args.word:
        ...
        for card in cards:
            if len(card) == 1:
                ...
                build_and_save(
                    card[0], args.language, file_name=card[1], dir_name=args.output_dir
                )
