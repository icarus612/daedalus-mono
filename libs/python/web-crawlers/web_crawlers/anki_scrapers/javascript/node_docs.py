...
should_remove_start = len(argv) <= 2 or argv[2].lower() not in [
    "0",
    "false",
    "f",
    "n",
...
header = list(
    filter(
        lambda x: any([len(i.find_all("code")) > 0 for i in x]),
        [page.find_all(f"h{i}") for i in range(3, 6)],
    )
)[0]
for el in header:
    item = el.find("code")
        ...
        for sib in el.next_siblings:
            match sib.name:
                case "p":
                    ...
                    cut_len = (
                        len(f"The {part} {prop_type} ") if should_remove_start else 0
                    )
