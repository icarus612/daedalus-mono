...
class Runner:
    ...
    def look_around(self, node):
        row, col = node.value
        for i in self.open_nodes:
            if i.value[0] - 1 == row and i.value[1] == col:
                node.add_child(i)
            elif i.value[0] + 1 == row and i.value[1] == col:
                node.add_child(i)
            elif i.value[1] - 1 == col and i.value[0] == row:
                node.add_child(i)
            elif i.value[1] + 1 == col and i.value[0] == row:
                node.add_child(i)
    ...
    def build_path(self, path="x"):
        ...
        maze = self.maze
        if (
            path == maze.start_char
            or path == maze.end_char
            or path == maze.wall_char
