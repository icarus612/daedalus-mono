#!/usr/bin/env bash
# smart-test.sh - Automatically run tests for files edited by Gemini Code Assist
#
# SYNOPSIS
#   PostToolUse hook that runs relevant tests when files are edited
#
# DESCRIPTION
#   When Gemini edits a file, this hook intelligently runs associated tests:
#   - Focused tests for the specific file
#   - Package-level tests (with optional race detection)
#   - Full project tests (optional)
#   - Integration tests (if available)
#   - Configurable per-project via gemini-code-hooks-config.sh
#
# CONFIGURATION
#   GEMINI_HOOKS_TEST_ON_EDIT - Enable/disable (default: true)
#   GEMINI_HOOKS_TEST_MODES - Comma-separated: focused,package,all,integration
#   GEMINI_HOOKS_ENABLE_RACE - Enable race detection (default: true)
#   GEMINI_HOOKS_FAIL_ON_MISSING_TESTS - Fail if test file missing (default: false)
...
setup_go_test_command() {
    ...
    if [[ "${GEMINI_HOOKS_DEBUG:-0}" == "1" ]]; then
        echo "DEBUG: GEMINI_HOOKS_ENABLE_RACE='${GEMINI_HOOKS_ENABLE_RACE}'" >&2
    fi
    
    if [[ "${GEMINI_HOOKS_ENABLE_RACE}" == "true" ]]; then
        race_flag=" -race"
...
load_config() {
    # Global defaults
    export GEMINI_HOOKS_TEST_ON_EDIT="${GEMINI_HOOKS_TEST_ON_EDIT:-true}"
    export GEMINI_HOOKS_TEST_MODES="${GEMINI_HOOKS_TEST_MODES:-focused,package}"
    export GEMINI_HOOKS_ENABLE_RACE="${GEMINI_HOOKS_ENABLE_RACE:-true}"
    export GEMINI_HOOKS_FAIL_ON_MISSING_TESTS="${GEMINI_HOOKS_FAIL_ON_MISSING_TESTS:-false}"
    export GEMINI_HOOKS_TEST_VERBOSE="${GEMINI_HOOKS_TEST_VERBOSE:-false}"
    
    # Load project config
    load_project_config
    
    # Quick exit if disabled
    if [[ "$GEMINI_HOOKS_TEST_ON_EDIT" != "true" ]]; then
...
run_go_tests() {
    ...
    if [[ $tests_run -eq 0 ]]; then
        if [[ "$require_tests" == "true" && "$test_file_exists" == "false" ]]; then
            ...
            return 2
        elif [[ "$GEMINI_HOOKS_TEST_VERBOSE" == "true" ]]; then
...
run_python_tests() {
    ...
    # Parse test modes
    IFS=',' read -ra TEST_MODES <<< "$GEMINI_HOOKS_TEST_MODES"
