...
1. Re-read ~/claude-code/CLAUDE.md RIGHT NOW
...
- Run `~/claude-code/hooks/smart-lint.sh` directly to see current state
- If ANY issues exist, they MUST be fixed before proceeding
- Check `~/claude-code/hooks/violation-status.sh` if it exists
...
- `~/claude-code/hooks/smart-lint.sh` for automatic detection
