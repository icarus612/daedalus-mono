...
#
# CONFIGURATION
#   Project-specific overrides can be placed in claude-code-hooks-config.sh
...
should_skip_file() {
    ...
    # Check claude-code-hooks-ignore if it exists
    if [[ -f "claude-code-hooks-ignore" ]]; then
        while IFS= read -r pattern; do
            ...
            if [[ "$file" == $pattern ]]; then
                log_debug "Skipping $file due to claude-code-hooks-ignore pattern: $pattern"
                return 0
            fi
        done < "claude-code-hooks-ignore"
    fi
...
load_config() {
    ...
    if [[ -f "claude-code-hooks-config.sh" ]]; then
        source "claude-code-hooks-config.sh" || {
            log_error "Failed to load claude-code-hooks-config.sh"
            exit 2
        }
    fi
